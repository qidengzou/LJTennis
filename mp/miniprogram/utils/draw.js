...
/**
 * 小组积分表。判定顺序：胜场 → 净胜盘 → 净胜局（`PRD.md` §5）。
 * **只有这三档** —— 全平就是并列，系统不往下判，出线由主办方在签表上定。
 * 这条规则要显示在表下面，不能藏进规则页。
 *
 * ⚠️ **净胜盘那一档还没做**（缺 setsWon/setDiff，见 SPEC.md §5.1），
 * 现在只比了胜场和净胜局，中间少一档。
 *
 * ⚠️ **并列还没有并列名次**：rank 恒为 i+1（见 SPEC.md §5.10）。
 *
 * 末档的 seq **不是第四条判定规则**，只是「表里谁先显示」，
 * 让同分行的顺序稳定、不随调用次数变。**别在这里补随机数** ——
 * 抽签事后没法复算，谁也说不清那次抽的是什么。
 */
