...
根目录是规格与设计稿，`mp/` 是小程序工程，`adminweb/` 是组织者网页后台，
`api/` 是契约。**都在同一个仓里**，理由见「后台硬约束」。
...
| 后台的目录结构、数据层、构建与部署 | `adminweb/CLAUDE.md` |
...
> ⚠️ **`adminweb/` 现在还是空目录。** 上面这条是起工程时必须配好的门，
> 不是现在能跑的命令 —— 别照着执行然后以为坏了。
...
`adminweb/` 是组织者用的**网页后台**，React + Vite + TypeScript，
**同仓子目录**，和 `mp/` 平级。细则在 `adminweb/CLAUDE.md`，这里只放
在别的目录里干活也得知道的五条。

- **不拆成子仓库 / submodule。** `api/types.ts` 是实体与状态机的真身，
  后台必须消费它 —— 同仓才能让 `tsc` 直接编译过去，**类型不同步就编译不过**。
  拆了仓这道门就没了，只能靠人记得。这个仓已经为同样的问题吃过一次亏：
  `mp/miniprogram/utils/entry.js` 和 `mp/cloudfunctions/api/shared/entry.js`
  必须字节一致，为此专门写了 `check-shared.js` —— 而那两个还在同一个仓里。
- **后台不新开后端。** 走同一个 `api` 云函数分发器，只是换个客户端
  （`@cloudbase/js-sdk` 而不是 `wx.cloud`）。别为后台写第二套 handler。
- **单位是 px，不是 rpx。** 颜色令牌两端共用一套，尺寸令牌各一套 ——
  桌面控件 36px，`88rpx`（44pt）那条是给手指的。见 `design/DESIGN.md` §1。
- **后台也不做深色模式**，全站同一条。
- **端的分界线是「现场 vs 赛前」，不是「简单 vs 复杂」**（`PRD.md` §2）。
  记分、改比分、处理争议留在小程序里 —— 人站在球场边。
  别因为后台屏大就把现场功能挪过去。

> 🚧 **写后台页面之前，`admin.*` 的鉴权必须先补上。** 见「不要碰」最后一条。
> 今天这个洞暴露面小，是因为**根本没有 admin 客户端**；后台一上公网静态托管，
> 就变成「一个谁都能调的路由，返回明文手机号」。顺序不能反。
...
- `admin.*` 路由**目前没有鉴权**，而 `users.phoneEncrypted` 存的是**明文**
  （字段名在撒谎）。两件事**必须一起补**，见 `SPEC.md` §5.2 ——
  单独做任何一件都等于没做。
  **`adminweb/` 立项之后这条从「待修」升级成「阻塞」**：
  后台一上线，这个洞就从「理论上可调」变成「公网上有个页面在用它」
