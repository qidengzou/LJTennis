...
OBS 的 browser source 没法登录，所以按**场次编号**读比分这一条也必须免鉴权
（`PRD.md` §6）。它和上面三个不是一回事：那三个是拉新落地页，这一条是给
直播设备用的只读窗口。

| 接口 | 用途 |
|---|---|
| `live.match?code=260910-0012` | 按场次编号取当前比分。**只对 `status === 'live'` 的场次返回** |

两条约束，一条都不能松：

- **只返回这一场需要显示的东西** —— 双方名字、已完成盘的局分、当前分、发球方。
  不返回手机号，也不返回选手 id 之外的任何资料
- **比赛一结束就查不到**。编号是「日期 + 4 位序号」，能猜；只在 `live` 时开放，
  它就只是一个当下的窗口，而不是一把能翻遍历史场次的钥匙
...
| `match.release` | **是当前记分方本人** —— 交出记分权让别人接手。交的是记分权不是比分；客户端要先把本地未同步的分推上去再调 |
...
| 层 | 回答什么 | 谁有 |
|---|---|---|
| `PlatformRole` | 你在这个产品里是谁 | 建俱乐部只有平台管理员能做 |
| `ClubRole` | 你在**某一个**俱乐部里是谁 | 该俱乐部的数据只对它的管理员开 |

**两层刻意不互相包含**：平台管理员能建俱乐部、能指定 owner，
但**看不到**某场赛事报名者的手机号 —— 「能开店」和「能翻店里的客户名单」
不是同一个权限。规则写在 `api/types.ts` 的 `PlatformRole` 上；
谁能看到手机号见 `PRD.md` §9。

> 🚧 **以上是要做的，不是现在的。`admin.*` 现在没有任何鉴权校验。**
> 和 `users.phoneEncrypted` 存明文两件事必须一起补，见 `SPEC.md` §5.2。
> **后台上公网之前这条必须先落**。
...
这一组不走 REST 路径，用云函数分发器的 action 名（下面那条注意）。
**现在只有两条真实存在的**：

| Action | 用途 | 状态 |
|---|---|---|
| `admin.initDb` | 建 9 个集合 | 有 |
| `admin.import` | 批量导入文档 | 有 |

后台 9 屏里除「扫码登录」外的 8 屏所需的读写接口**都还没定**
（屏的清单见 `SPEC.md` §2）。写后台页面时逐屏补进这张表，别在前端直连数据库。

> ⚠️ **这份文档写的是 REST 路径，实现是单云函数分发器**
> （`mp/cloudfunctions/api/index.js` 里 `'entry.create'` 这样的 action 名）。
> 两种写法指的是同一件事，但对不上号迟早会咬人 ——
> 后台接口按实现的写法记，老的那些暂时保持原样。
